...
namespace TXT2XML
{
    public class AhSchlagworte
    {
        private readonly char[] codes = new char[0];

        public int NumberOfKeywords => codes.Length;
        public string RawKeywordString { get; } = "";
        public List<string> Keywords => PrepareKeywords();

        public AhSchlagworte(string raw)
        {
            RawKeywordString = raw.Trim();
            codes = RawKeywordString.ToCharArray();
        }

        public override string ToString() { return RawKeywordString; }

        private string CodeToName(char c)
        {
            switch (c)
            {
                case 'A':
